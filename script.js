      ...
      // --- Input & Mechanical UI Logic ---
        ...
        // UI Sound Function (Safely mapped to existing audioCtx)
      ...
      // Nametag typing effects & Event Shielding
        ...
        nameInput.addEventListener('input', (e) => {
            ...
            // YOUR IDEA: Force uppercase and strip out numbers/symbols/spaces
        ...
        // THE FIX: Build a soundproof wall so the game engine can't hear you type WASD
        ...
        // Engage Button with Screen Shake
        ...
        // The Master Switch (Nuclear display:none method)
        document.addEventListener('pointerlockchange', () => {
            if (document.pointerLockElement === document.body) {
                uiContainer.style.display = 'none'; // Erases menu completely
                gameActive = true;
                if (startTime === 0) startTime = Date.now();
                prevTime = performance.now();
            } else if (!gameWon) {
                uiContainer.style.display = 'flex'; // Brings menu back safely
        ...
        function update() {
            ...
            enemies.forEach((enemy, index) => {
                ...
                // Distance is calculated exactly ONCE here:
                const dist = camera.position.distanceTo(enemy.position); 
                if (dist < closestDist) closestDist = dist;  

                // --- NEW V3 DEATH SCREEN TRIGGER ---
                if (!gameWon && dist < 3.0 && gameActive) { 
                    ...
                    // Show Screen over everything
                    document.getElementById('death-screen-ui').style.display = 'block';
                }
            }); // End of enemies loop (Only ONE of these!)

            if (!gameWon && closestDist < 12) { camera.position.x += (Math.random() - 0.5) * ((12 - closestDist) * 0.02); if (!hasPlayedSting) { playSting(); hasPlayedSting = true; } } else { hasPlayedSting = false; }
